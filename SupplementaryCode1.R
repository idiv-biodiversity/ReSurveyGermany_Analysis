...
# Nature
#
# Author: Helge Bruelheide, 26.02.2022
...
DT <- read.csv(file="DT4_no_cryp_layer.csv")
DT <- data.table(DT)
str(DT) #610313 obs. of  5 variables:
# DT holds 
# RELEVE_NR: ID within project
# LAYER:          code for the layer
# TaxonName:      Harmonized taxonomic name
# Cover_Perc:     Per cent cover of the species in the plot
# RELEVE_NR_comb: ID across all projects, links DT to header

# Load header data
header <- read.csv("header_resurvey4.csv")
str(header) # 23641 obs. of  77 variables:

# Link DT to header
index1 <- match(DT$RELEVE_NR, header$RELEVE_NR_comb)

length(unique(header$project_perm_plot_ID))
# In total, there are 7738 time series

# Combine cover of species across layers
DT2 <- DT[,list(Cover_Perc=combine.cover(Cover_Perc)), by=list(RELEVE_NR,TaxonName)]
str(DT2)
# 583395 obs. of  3 variables

# Split RELEVE_NR_comb in DT into project-specific RELEVE_NR and project ID
DT2$project <- as.numeric(tstrsplit(DT2$RELEVE_NR,"_")[[1]])
DT2$RELEVE_NR <- as.numeric(tstrsplit(DT2$RELEVE_NR,"_")[[2]])

length(unique(DT2$project))
...
for (i in c(43:length(unique(DT2$project)))){
  ...
  species.long <- DT2[DT2$project==i,]
  # turn the long format into a RELEVE_NR by TaxonName matrix
  species <- acast(species.long, RELEVE_NR~TaxonName, value.var ="Cover_Perc", fill=0)
  # select the header data from a given project
  env <- header[header$project==i,]
  ...
  # set of plots in time if combined with RS_PROJECT (see project_perm_plot_ID ). 
  # Several plots in the same year might have the same RS_PLOT code if they have 
  # to be summarised for temporal comparisons.
  perm_plot_ID <- env$RS_PLOT
  # plot.list holds the list of all perm_plot_ID
  plot.list <- sort(names(table(perm_plot_ID))[table(perm_plot_ID)>1])
  ...
  # loop for all perm_plot_ID
  for (j in 1:length(plot.list)){
    target.year <- year[perm_plot_ID %in% plot.list[j],]
    if (length(unique(target.year))>1){
      ...
      species.target.plot <- species[perm_plot_ID %in% plot.list[j],]
      ...
      for (k in 1:(length(n)-1)){
        ...
        richness.change.list <- rbind(richness.change.list, data.frame(
          project=Projects$RS_PROJECT[i],perm_plot_ID=plot.list[j],
          from.n=as.numeric(n[k+1]), to.n=as.numeric(n[k]), from=as.numeric(names(n)[k+1]),
          to=as.numeric(names(n)[k]),
...
# Make a unique ID from project and RS_PLOT (=perm_plot_ID) 
richness.change.list$project_perm_plot_ID <- paste(richness.change.list$project,richness.change.list$perm_plot_ID, sep="_")
...
species.change.list2$project_perm_plot_ID <- paste(species.change.list2$project,
                                                   species.change.list2$perm_plot_ID, sep="_")
str(species.change.list2) #458311 in_MS Results, Methods

### Step. 3: Calculation of change at the plot level
# The following code produces Fig. ###

# Histogram on the number of intervals per time series
table(table(paste(richness.change.list$project,richness.change.list$perm_plot_ID)))
...
# x=log.shannon.change for Fig. 3a
# x=log.evenness.change for Fig. 3b
# x=curve.diff for Fig. 3c
# x=diff.gains.losses for Fig. 3d
# x=mean.cover.change for Fig. 3e
# x=median.cover.change for Fig. 3f
...
barplot(-(0.5-change2b$est.binom),width=1, space=0,
        col=col.vector3[change2b$eunis.class], offset=0.5, xaxt="n", add=T) 
...
legend("topleft", bty="n", cex=2,
       ...
       legend=c("Species with negative mean change", "Species with positive mean change"))
mtext("b",side=2,line=4, at=1.05, las=1, cex=3)
dev.off()


# Calculate the Gini coefficients
Gini(x = abs(change.neg$mean.absolute.change),conf.level=0.95, R=1000, 
     type="perc", unbiased=T) 
'     gini    lwr.ci    upr.ci 
0.6915324 0.6597624 0.7179777  '

Gini(x = abs(change.pos$mean.absolute.change),conf.level=0.95, R=1000, 
     type="perc", unbiased=T) 
'     gini    lwr.ci    upr.ci 
